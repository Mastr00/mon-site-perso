...
import { Sun, Cloud, CloudRain, CloudSnow, CloudLightning, CloudFog, CloudDrizzle, Wind, MapPin, Loader2 } from 'lucide-react';

// Maps WMO code to Lucide Icon and background color
const getWeatherInfo = (code) => {
    if (code === 0) return { icon: <Sun size={32} className="text-yellow-400" />, text: "Ciel clair", color: "from-blue-400 to-cyan-400" };
    if ([1, 2, 3].includes(code)) return { icon: <Cloud size={32} className="text-gray-300" />, text: "Nuageux", color: "from-slate-500 to-slate-400" };
            ...
            try {
                ...
                if (!fallbackName.includes("défaut")) {
                    try {
                        ...
                        city = geoData.address.city || geoData.address.town || geoData.address.village || "Position Actuelle";
