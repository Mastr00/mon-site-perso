...
export default function CryptoWidget() {
    ...
    return (
        <motion.div 
            ...
            className="h-full p-6 bg-white dark:bg-[#0F172A] border border-slate-200 dark:border-amber-500/20 rounded-2xl shadow-sm dark:shadow-[0_0_15px_rgba(245,158,11,0.1)] flex flex-col justify-between"
        >
            <div className="flex justify-between items-center mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
                <h2 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <Bitcoin className="text-amber-500" size={20} /> Crypto Markets
                </h2>
                <div className="text-xs text-slate-400 flex items-center gap-1 font-medium bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">
                        ...
                        {cryptoData.map(coin => (
                            <div key={coin.symbol} className="flex justify-between items-center group">
                                <div className="flex items-center gap-2">
                                    <div className={`w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-bold text-xs ${coin.color}`}>
                                        {coin.symbol}
                                    </div>
                                ...
                                <div className="text-right">
                                    <div className="font-bold text-slate-900 dark:text-white font-mono tracking-tight">
