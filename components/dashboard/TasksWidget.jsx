      ...
      <ul className="space-y-3 overflow-y-auto pr-1 custom-scrollbar max-h-60">
        <AnimatePresence>
          {tasks.map((task) => (
            ...
            >
              ...
              <span
                className={`flex-1 font-medium text-sm ${task.done ? 'line-through text-cyber-400' : 'text-cyber-700 dark:text-cyber-200'}`}
              >
                {task.text}
              </span>
        ...
        {tasks.length === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex flex-col items-center justify-center py-8 text-cyber-400"
          >
