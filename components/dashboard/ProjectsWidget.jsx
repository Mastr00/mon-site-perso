        ...
        <div className="bg-cyber-50 dark:bg-cyber-900 p-6 rounded-2xl shadow-sm h-full flex flex-col border border-cyber-200 dark:border-cyber-800 relative overflow-hidden group">
            ...
            <div className="flex justify-between items-center mb-6 relative z-10">
                <h2 className="text-xl font-bold text-cyber-950 dark:text-cyber-100 flex items-center gap-2">
                    <span className="p-2 bg-cyber-accent/10 rounded-lg text-cyber-accent">
                        🚀
                    </span>
                    Projets
                </h2>
            ...
            <div className="flex-grow overflow-y-auto relative z-10 scrollbar-hide">
                {!isAdding ? (
                    <div className="flex flex-col gap-3">
                        {tempProjects.length > 0 && (
                            <div className="mb-4 space-y-2">
                                <h3 className="text-xs font-semibold text-cyber-400 uppercase tracking-wider">Nouveaux ajouts</h3>
                                {tempProjects.map(p => (
                                    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} key={p.id} className="p-3 rounded-lg bg-cyber-100 dark:bg-cyber-800 border border-cyber-200 dark:border-cyber-700 flex items-center justify-between">
                                        <span className="font-medium text-cyber-950 dark:text-cyber-100 text-sm truncate">{p.title}</span>
                    ...
                    >
                        <div>
                            <label className="block text-xs font-bold text-cyber-500 uppercase mb-1 ml-1">Titre</label>
                            <input required name="title" type="text" className="w-full px-4 py-2 text-sm rounded-xl bg-cyber-100 dark:bg-cyber-800 border border-cyber-200 dark:border-cyber-700 focus:ring-2 focus:ring-cyber-accent outline-none transition-all text-cyber-950 dark:text-cyber-100" placeholder="Nom du projet..." />
                        </div>

                        <div>
                            <label className="block text-xs font-bold text-cyber-500 uppercase mb-1 ml-1">Description</label>
                            <textarea required name="description" rows="2" className="w-full px-4 py-2 text-sm rounded-xl bg-cyber-100 dark:bg-cyber-800 border border-cyber-200 dark:border-cyber-700 focus:ring-2 focus:ring-cyber-accent outline-none transition-all resize-none text-cyber-950 dark:text-cyber-100" placeholder="Court résumé..." />
                        </div>
