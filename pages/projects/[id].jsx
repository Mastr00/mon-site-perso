      ...
      <div className="min-h-screen bg-cyber-50 dark:bg-cyber-950 dark:bg-cyber-950 text-cyber-700 dark:text-cyber-100 py-16 px-4 md:px-8 relative overflow-hidden bg-grid">
         ...
         <div className="max-w-4xl mx-auto relative z-10 pt-10">
            ...
            >
               ...
               <Image src={project.image} alt={titleStr} fill className="object-cover group-hover:scale-105 transition-transform duration-700" priority />
               ...
               <div className="absolute bottom-6 left-6 md:bottom-10 md:left-10 z-20">
                   ...
                   <div className="flex flex-wrap gap-2">
                       {project.tags.map((tag) => (
                           <span key={tag} className="px-3 py-1 bg-cyber-200/50 dark:bg-cyber-800/80 backdrop-blur-md border border-white/20 rounded-[3px] text-sm font-semibold text-white shadow-sm">
                               {tag}
                           </span>
                       ))}
                   </div>
            ...
            <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay:0.2}} className="flex flex-wrap gap-4 mb-12">
               {project.demo && project.demo !== "#" && (
                   <a href={project.demo} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-6 py-3 bg-cyber-cta hover:bg-cyber-accent text-white font-bold rounded-sm shadow-[0_4px_15px_rgba(56,189,248,0.15)] transition-all hover:-translate-y-0.5">
                       <ExternalLink size={20} /> {t.portfolio.demo}
                   </a>
               )}
               {project.repo && (
                   <a href={project.repo} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-6 py-3 bg-cyber-50 dark:bg-cyber-900 border border-cyber-200 dark:border-cyber-800 text-cyber-950 dark:text-cyber-100 font-bold rounded-sm hover:border-cyber-accent/50 hover:bg-cyber-200 dark:hover:bg-cyber-800 transition-all hover:-translate-y-0.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                {/* L'idée / Présentation */}
                <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay:0.3}} className="md:col-span-2 bg-cyber-50 dark:bg-cyber-900/80 backdrop-blur-sm border border-cyber-200 dark:border-cyber-800 rounded-sm p-8 hover:border-cyber-accent/30 transition-colors">
                ...
                {/* Les Défis */}
                <motion.div initial={{opacity:0, x:-20}} animate={{opacity:1, x:0}} transition={{delay:0.4}} className="bg-cyber-50 dark:bg-cyber-900/80 backdrop-blur-sm border border-red-500/20 rounded-sm p-8 hover:border-red-500/50 transition-colors relative h-full">
                ...
                {/* La Solution */}
                <motion.div initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} transition={{delay:0.5}} className="bg-cyber-50 dark:bg-cyber-900/80 backdrop-blur-sm border border-green-500/20 rounded-sm p-8 hover:border-green-500/50 transition-colors relative h-full">
                    <h2 className="text-2xl font-mono font-bold text-cyber-950 dark:text-cyber-100 mb-4 flex items-center gap-3 relative z-10">
            ...
            {project.hardware && project.hardware.length > 0 && (
                <motion.div initial={{opacity:0, y:20}} animate={{opacity:1, y:0}} transition={{delay:0.6}} className="bg-cyber-50 dark:bg-cyber-900/80 backdrop-blur-sm border border-cyber-accent/20 rounded-sm p-8 mb-16 relative">
                    ...
                    <div className="flex flex-wrap gap-3 relative z-10">
                        {project.hardware.map((item, idx) => (
                            <div key={idx} className="flex items-center gap-2 bg-cyber-100 dark:bg-cyber-800 border border-cyber-200 dark:border-cyber-700 px-3 py-2 rounded-[3px] text-cyber-700 dark:text-cyber-100 text-sm font-semibold hover:border-cyber-accent/50 transition-colors">
